...
func (repo *TicketRepositoryImplementation) AddTicket(tx *sql.Tx, ctx context.Context, ticket *entity.Ticket) {
	defer helper.ShouldRollback(tx)
	res, err := tx.ExecContext(ctx, "INSERT INTO ticket(bus_id,driver_id,customer_id,departure_place,arrival_place,price,arrived) VALUES (?,?,?,?,?,?)", ticket.BusId, ticket.DriverId, ticket.CustomerId, ticket.DeparturePlace, ticket.ArrivalPlace, ticket.Price, ticket.Arrived)
	helper.PanicIfError(err)
